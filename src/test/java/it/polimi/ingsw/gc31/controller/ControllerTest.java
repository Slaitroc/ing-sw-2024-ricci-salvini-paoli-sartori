...
class ControllerTest {
    ...
    void generateTokenTest() {
        ...
        //Simulate the token generation of new clients, the value of the tokens should be between
        //0 and 999. After every token generation the size of the newConnections map should be
        //the old one +1
        ...
        while(i < 999){
            ...
            i++;
            //System.out.println("Inserito il token " + i + " all'interno della mappa newConnections");
        }

        //Checks if the correct (only one possible) value of the token is generated. There can't be any
    ...
    void sendTokenTest() {
        ...
        //Checks if the correct flow of instructions is executed during the process to send
    ...
    void connectTest() {
        ...
        //The mockClient1 (token = 100) disconnected, the mockClient2 (still token = 100) is the
        // new VirtualClient of the user. mockClient2 tries to reconnect so the first if statement
        // should be executed. The value associated to the token = 100 should be replaced with the new VirtualClient and
        ...
        //If the token is not contained but the username of the new client is already contained in the nicknames list
        //the else branch should be executed and false should be returned, without adding the client
        //to any map
        VirtualClient mockClient3 = Mockito.mock(VirtualClient.class);
    ...
    void rejoinTest() {
        ...
        //If the token of the client was in the disconnected map the user can choose to rejoin or not
        //If the boolean value of the rejoin method is true the player wants to rejoin and the if branch should be executed
        assertDoesNotThrow( () -> controller.rejoin("username1", 100, true));

        VirtualClient mockClient2 = Mockito.mock(VirtualClient.class);
        controller.newConnections.put(200, mockClient2);

        //If the method rejoin is invoked with the boolean parameter false the else branch is executed
        ...
        //If a clients tries to rejoin with the boolean parameter false but with the boolean
        //parameter false the method doesn't add the client to any map and send a WrongUsernameObj
    ...
    void createGameTest() {
        ...
        //Checks if the correct instructions are executed when the maxNumberPlayer parameter is out of correct bounds
    ...
    void joinGameTest() {
        ...
        //If the client tries to enter a game with the wrong gameId the Controller only sends
        //to him a GameDoesNotExistObj
        assertDoesNotThrow( () -> controller.joinGame("username2", 1));

        //If the client tries to enter a game with the correct gameId, and the game is not full, the client
        ...
        //If the client tries to enter to a game that is already full the Controller only sends
        //him a GameIsFullObj
    ...
    void getRightConnectionTest() {
        ...
        //Check if the value returned is null when the token is not contained in the map
    ...
    void updateHeartBeatTest() {
        ...
        //Check if the updateHeartBeat execute the correct instructions when invoked for a client
        //that is in the map
     ...
     * It is invoked only when the methodCheckHeartBeats method finds a user, that is already in a game,
     * that disconnected
     */
    @Test
    void disconnectTest() {
        ...
        //Check if the disconnect map size changed accordingly to the method instructions
