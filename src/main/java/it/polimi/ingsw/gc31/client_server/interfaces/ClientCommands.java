...
/**
 * Defines the methods executable by all the {@link it.polimi.ingsw.gc31.Client}. These methods are involved with the {@link UI} classes which directly communicate
 * with the user. Based on the input received from the user a specific method is invoked by the{@link UI}.
 *
...
public interface ClientCommands {
    /**
     * Sets the {@link it.polimi.ingsw.gc31.Client} attribute of the {@link UI} to the reference received as a parameter.
     * Can be invoked by either the {@link it.polimi.ingsw.gc31.Client#main(String[])} method or the
     * {@link GUI#runUI()} method.
     *
     * @param ui is the reference of the {@link UI} chosen by the client to be set as the new value of the client's attribute.
     * @see it.polimi.ingsw.gc31.view.tui.TUI
     * @see GUI
     */
    void setUI(UI ui);
    ...
    /**
     * Sets the private values of tempToken and token to the values received as a parameter. Is implemented both in
     * {@link it.polimi.ingsw.gc31.client_server.rmi.RmiClient} and {@link it.polimi.ingsw.gc31.client_server.tcp.TCPClient}.
     * If the temporary value is true the method only sets the value of tempToken.
     * If the temporary value is false the method sets both the values of token and tempToken, also writes on the file
     * the new value of the token received.
     *
     * @param token     is the new value of the token to be set.
     * @param temporary indicates if the token to set is the temporary one or both of them.
     *
     * @see Token
...
//    boolean hasToken();
    ...
    /**
     * Return the reference of the {@link it.polimi.ingsw.gc31.Client} {@link Token}. The {@link it.polimi.ingsw.gc31.Client}
     * can be either a {@link it.polimi.ingsw.gc31.client_server.rmi.RmiClient} or a {@link it.polimi.ingsw.gc31.client_server.tcp.TCPClient}.
     *
     * @return the reference of the {@link Token} of the {@link it.polimi.ingsw.gc31.Client}.
     */
    Token getToken();
...
//    int readToken() throws NumberFormatException, NoTokenException;
    ...
    /**
     * Sends a new {@link ReconnectObj} to the {@link Controller}. The boolean parameter indicates if the user
     * wants to reconnect or not to the match where the user was before disconnecting.
     *
     * @param reconnect         indicates if the user wants to reconnect to the match or not.
