 ...
 * It can be registered to a GsonBuilder when you have to deserialize a list of the type List<Resources>.
 * Use:
 * Gson gson = new GsonBuilder().registerTypeAdapter(new TypeToken<List<Resources>>(){}.getType(), new ListResourcesEnumAdapter()).create();
 */
public class ListResourcesEnumAdapter implements JsonDeserializer<List<Resources>> {
     ...
     * @return it returns the deserialized object. It returns a list of the type List<Resources>
    ...
    public List<Resources> deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
        ...
        for (JsonElement el : jsonArray) {
            // Convert the extracted string from jsonArray into its corresponding Resources value
